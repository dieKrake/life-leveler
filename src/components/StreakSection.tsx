        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            >
              {isStreakExpanded ? (
                <>
                  {t("streak.less")} <ChevronUp className="h-4 w-4" />
                </>
              ) : (
                <>
                  {t("streak.moreDetails")} <ChevronDown className="h-4 w-4" />
                </>
              )}
            </Button>
          </div>
        ...
        <CardContent>
          ...
          >
            ...
            >
              ...
              <p className="text-sm text-slate-400">{t("streak.daysInARow")}</p>
            </motion.div>
            <motion.div
              initial={{ opacity: 0, x: 20 }}
              animate={{ opacity: 1, x: 0 }}
            ...
            {isStreakExpanded && streakMultipliers && (
              ...
              >
                ...
                <motion.div
                  ...
                  transition={{
                    duration: 0.3,
                    delay: streakMultipliers.length * 0.1 + 0.2,
                  }}
                >
                  <p className="text-sm text-blue-300">
                    <strong>{t("streak.tip")}</strong> {t("streak.tipText")}
                  </p>
                </motion.div>
              </motion.div>
            )}
