-- Fix all remaining XP/Gems issues
-- Migration: 20251011240000_fix_all_remaining_issues.sql

-- 1. Update check_and_reverse_challenges_on_todo_uncomplete to NOT remove challenge rewards
-- Since we now have a claim system, challenges don't automatically award rewards
...
BEGIN
  ...
  FOR v_challenge IN
    SELECT uc.*, c.condition_type, c.target_metadata, c.xp_reward, c.gem_reward
    FROM user_challenges uc
    JOIN challenges c ON c.id = uc.challenge_id
    WHERE uc.user_id = p_user_id
      AND uc.expires_at > NOW()
  LOOP
    -- Store if challenge was completed before reversal
    ...
    -- REMOVED: No longer remove XP/gems from player_stats since challenges use claim system
    -- REMOVED: No longer delete challenge_completions since they're only created on claim
    
  END LOOP;
END;
$$;

-- 2. Update check_and_update_challenges_on_todo_complete to NOT award rewards automatically
...
BEGIN
  ...
  -- Loop through all active challenges for this user
  FOR v_challenge IN
    SELECT uc.*, c.condition_type, c.target_value, c.target_metadata, c.xp_reward, c.gem_reward
    FROM user_challenges uc
    JOIN challenges c ON c.id = uc.challenge_id
    WHERE uc.user_id = p_user_id
      AND uc.expires_at > NOW()
  ...
  LOOP
    ...
    IF v_new_progress >= v_target_value THEN
      ...
      -- REMOVED: No automatic XP/gems rewards - user must claim them
    END IF;
  END LOOP;
END;
$$;
