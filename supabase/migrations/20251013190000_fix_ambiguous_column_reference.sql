-- Migration: Fix ambiguous column reference in get_player_stats_with_level_info
-- Date: 2025-10-13 19:00:00
-- Description: Fix the "level" column ambiguity by using table aliases
...
BEGIN
    ...
    -- Get current level XP requirement (use table alias to avoid ambiguity)
    SELECT l.xp_required INTO current_level_xp
    FROM levels l
    WHERE l.level = user_level;
    
    -- Get next level XP requirement (null if at max level 10)
    IF user_level < 10 THEN
        SELECT l.xp_required INTO next_level_xp
        FROM levels l
