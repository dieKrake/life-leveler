-- Migration: Fix Achievement XP to use effective_xp (total_xp - prestige_base_xp)
-- Date: 2026-03-28 13:00:00
-- Description: 
--   Fixes achievement progress calculation for XP-based achievements.
--   Currently uses ps.xp (level XP) instead of effective XP since prestige.
--   Correct formula: effective_xp = total_xp - prestige_base_xp

-- 1. Fix get_user_achievements_with_progress to use total_xp
    ...
    SELECT 
        ...
        CASE 
            ...
            WHEN a.condition_type = 'reach_value' AND a.category = 'xp' THEN 
                LEAST(100.0, (effective_xp::NUMERIC / a.condition_value::NUMERIC) * 100)
...
-- 2. Fix check_and_update_achievements to use total_xp
...
BEGIN
    -- Get current player stats including total_xp and prestige_base_xp
    SELECT level, total_xp, gems, COALESCE(prestige_base_xp, 0)
